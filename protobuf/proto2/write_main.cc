...
void PromptForAddress(mytest_proto2::Person* person) {
    if (!person){
        return;
    }
    ...
    while (true){
        ...
        mytest_proto2::Person::PhoneNumber* phone_number = person->add_phones();
        ...
        if (type == "mobile") {
            phone_number->set_type(mytest_proto2::Person::MOBILE);
        }else if (type == "home") {
            phone_number->set_type(mytest_proto2::Person::HOME);
        }else if (type == "work") {
            phone_number->set_type(mytest_proto2::Person::WORK);
        }else {
