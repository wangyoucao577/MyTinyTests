...
核心文件为ipv6_to_ipv4_interface.c, 其中封装了`easy_getaddrinfo`, `ip_str_family`, `inet_ntop_ipv4_ipv6_compatible` 三个典型接口(support Windows, Linux, iOS and Android), 同时实现了`iOS`上获取本地网卡ip地址的`get_local_net`函数, 可供实际业务中需要时参考.
