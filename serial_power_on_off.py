...
def power_operate_com(comport, cmd):
    try:
        ...
        sys.stdout.write("Error: Can not open serial port " + str(comport) + "\n")
        #traceback.print_exc()
        
#Working start
def main():
    #sys.stdout.write("sys.version: " + str(sys.version) + "\n")  
    ...
    if (len(sys.argv) < 3 or (sys.argv[1] not in available_comports) or (sys.argv[2].upper() not in available_commands)) :
        ...
        sys.stdout.write("      python serial_power_on_off.py [COM1|COM2|...] [ON|OFF]\n")
        sys.stdout.write("Available Serial Ports: " + str(available_comports) + "\n")
    else:
